...
def analyze_videos(input_dir: Path) -> Tuple[List[Dict[str, Any]], int]:
    ...
    logger.success(
        f"Analysis complete. Total of {grand_total_frames} frames available across {len(video_metadata)} videos."
    )
    return video_metadata, grand_total_frames


def allocate_and_extract_frames(
    video_metadata: list, grand_total_frames: int, total_samples: int, output_dir: Path
) -> int:
    """
    ...
    logger.info(
        f"Found {num_videos} videos to process. Starting extraction with {max_processes} parallel processes..."
    )
    ...
    try:
        ...
        while True:
            ...
            frame_resized = cv2.resize(
                frame, (config.IMG_SIZE, config.IMG_SIZE), interpolation=cv2.INTER_AREA
            )
...
def extract_frames_for_model(
    ...
    if grand_total_frames < total_samples:
        logger.error(
            f"The total number of available frames ({grand_total_frames}) is less than the requested number of samples ({total_samples})."
        )
        raise typer.Exit(code=1)
